...
按是否查看代码分为
	黑盒测试：也称功能测试，测试中把被测的软件当成一个黑盒子，不关心盒子里的内部结构是什么， 只关心软件的输入数据和输出数据（不关心软件的内部结构和逻辑，只关注输入和输出是不是符合我们的需求）
		...
		黑盒测试的测试用例的设计方法：等价类，场景设计法，错误猜测法，边界值，因果图，正交排列法，按照需求     白盒测试的测试方法：逻辑覆盖，循环覆盖，路径覆盖
...
测试管理：
	测试需求的分析，测试策略制定 测试方案的设计 测试执行流程的设计  测试报告的输出

	1. 策略：用什么方法巧妙的去设计测试 设计性需求（非功能性需求，软件内部架构设计（不同的用户的软件设计得根据不同的群体设计） 可靠性需求，可测试性需求，性能需求，安全性需求；所有的需求都会转化为测试需求（根据这些用户的需求来设计测试用例），再根据测试需求定制测试策略（权衡这次设计需要突出哪个需求（看测试策略偏重哪一块，主要测哪一块）），  自动化脚本利用率越高，越有价值（兼容性可用自动化测试）

	2. 需求评审（大致确定出来哪些做（提炼出来），哪些需求不做（代价大，不合理）） ----- 非常重要
	了解需求之后，细化需求后提取测试对象，根据测试对象（针对系统哪一块进行测试）设计测试策略

	3. 转换为软件需求（功能需求，非功能需求，性能需求，安全性需求，扩展性需求，可靠性需求，可移植性需求，易用性需求，兼容性需求）
		测试应该尽早介入 软件测试模型（V模型，W模型） 软件开发模型（瀑布模型，螺旋模型，增量模型，敏捷开发模型）
		不段变化的软件模型和没有需求文档时，需要测试人员不断收集原始的客户需求（用敏捷开发模型）
	分析需求的具体做法： 1. 需求串讲（需求理解不一致，介绍需求被，内容，进行答疑）
						2. 验证需求
						3. 从设计需求中提取测试需求
	测试策略的制定：
		测试业务：一个新的app(功能测试，安全性测试，安装卸载测试，兼容性测试，界面测试）
				 老APP，功能都稳定了，要新加一个功能（测试侧重点放在功能测试，文档数据安全性也要测，再做个回归测试）
				 只修改了页面图标（侧重点就在界面测试，测试的侧重点一切以需求为主）
	具体实施：
		测试计划（测试流程的规划，人，负责模块），
